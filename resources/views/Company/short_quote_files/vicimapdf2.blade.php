    ...
    <table class="table table-bordered cusTable" style="margin-top: 100px;">
        <thead>
            ...
            <tr>
                ...
                <th class="page2_2"><p> Door Core </p></th>
        ...
        <tbody>
            @foreach($doorRows as $door)
                @include('Company.short_quote_files.door-row', ['hideCosts' => $HideCosts, 'doorType' => $doorType ?? 'vicaima'])
            @endforeach
            ...
            <tr>
                ...
                @if($HideCosts == 0)
                    <td class="tbl_bottom">{{ $currency }}{{ round($SumDoorsetPrice, 2) }}</td>
                    <td class="tbl_bottom">{{ $currency }}{{ round($SumIronmongaryPrice, 2) }}</td>
                @endif
                <td class="tbl_bottom">{{ $currency }}{{ round($Alltotalpriceperdoorset, 2) }}</td>
            </tr>
        </tbody>
    </table>
</body>
