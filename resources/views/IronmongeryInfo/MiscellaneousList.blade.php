    ...
    <div class="app-main__inner">
        <div class="card">
            <div id="accordion">
                <div class="row">
                    <div class="col-sm-6">
                        <div class="card-header"><h5 class="card-title">Miscellaneous List</h5></div>
                    </div>
                </div>
                            ...
                            <ul class="accordian_list">
                                <div class="row">
                                        @if(!empty($data) && count((array)$data)>0)
                                            ...
                                            @endphp
                                            @endif
                                                  @if($row->MiscellaneousCategory == $categoryIndexWithoutSpace)

                                                        <div class="col-md-3 col-sm-6 col-6">
                                                            <div class="product_holder">
                                                                <div class="product_img">
                                                                    <img src="{{url('uploads/miscellaneousInfo/'.$row->MiscellaneousImage)}}">
                                                                    <!-- <div class="zoom">
                                                                    <div class="zoom__inner">
                                                                    <img src="{{url('uploads/miscellaneousInfo/'.$row->MiscellaneousImage)}}" data-zoom="{{url('uploads/miscellaneousInfo/'.$row->MiscellaneousImage)}}" class="zoom__image" alt="image">
                                                                    </div>
                                                                </div> -->
                                                                </div>
                                                                <a class="product_name" href="#"><span>{{$row->MiscellaneousCode}}-</span> {{$row->MiscellaneousName}}</a>
                                                                <div class="product_face">
                                                                    <b>{{$row->MiscellaneousFireRating}}</b>
                                                                    <b>{{ $currency }}{{$row->MiscellaneousPrice}}</b>
                                                                    @if($row->MiscellaneousCategory == 'PushHandles')
                                                                        <b>PushPlates</b>
                                                                    @else
                                                                        <b>{{$row->MiscellaneousCategory}}</b>
                                                                    @endif

                                                            </div>
                                                            <a href="{{url('ironmongery-info/update-miscellaneous/'.$row->MiscellaneousGeneratedKey)}}" class="product_edit">Edit</a>
                                                            <a href="#" onclick="product_delete('{{$row->id}}','{{$row->MiscellaneousCategory}}')" class="product_delete">Delete</a>
                                                            </div>
                                                        </div>
		...
		if(Category=='LocksandLatches'){
		    ...
		    swal({
		    	...
		    	.then((result) => {
					if (result.value) {
						$.ajax({
							url:`{{url('ironmongery-info/miscellaneous-delete/${id}',)}}`,
							type:"POST",
