                ...
                <div class="form-row">
                    ...
                    <div class="col-md-12">
                        <div class="position-relative form-group d-flex">
                            ...
                            <input type="checkbox" name="FourSidedFrame" id="foursidedframe"
                                class="change-event-calulation form-control custom-checkbox"
                                style="margin: 2px -4px 10px 12px;border: 1px solid rgb(206, 212, 218);display: inline-block;height: 15px;width: 15px;"
                                value="1" @if (!empty($Item['FourSidedFrame']) && $Item['FourSidedFrame'] == 1) {{ 'checked' }} @endif>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="position-relative form-group">
                    ...
                    <div class="col-md-6">
                        <div class="position-relative form-group">
                            ...
                            <select name="intumescentLeafType" id="intumescentLeafType"
                                class="form-control intumescentLeafchange" required>
                                <option value="">Select Leaf Type</option>
                                @foreach ($leafTypeIntumescentseal as $row)
                                    <option value="{{ $row->id }}"
                                        @if (isset($Item['IntumescentLeafType'])) @if ($Item['IntumescentLeafType'] == $row->id){{ 'selected' }} @endif
                                        @endif>{{ $row->leaf_type_key }}
                                        ({{ $row->leaf_type_value }})</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="position-relative form-group">
                            ...
                            <select name="fireRating" id="fireRating" class="form-control change-event-calulation"
                                required>
                        ...
                        <input type="hidden" id="fireratingoldvalue"
                            value="{{ isset($Item['FireRating']) ? $Item['FireRating'] : '' }}">
                        <input type="hidden" name="savedfirerating" id="savedfirerating"
                            value="@if (isset($Item['FireRating'])) {{ $Item['FireRating'] }}@else{{ '' }} @endif">
                    </div>
                    <div class="col-md-6">
                        <div class="position-relative form-group">
                        ...
                        <input type="hidden" name="oldFirerating" id="oldFirerating"
                            value="@if (isset($Item['DoorsetType'])) {{ $Item['DoorsetType'] }}@else{{ '' }} @endif">
                    </div>
                    <div class="col-md-6">
                        <div class="position-relative form-group">
                            ...
                            <select required name="OpensInwards" id="OpensInwards" class="form-control">
                                ...
                                @foreach ($option_data as $row)
                                    @if ($row->OptionSlug == 'Opens_Inwards')
                                        <option value="{{ $row->OptionKey }}"
                                            @if (!empty($Item['OpensInwards'])) @if ($Item['OpensInwards'] == $row->OptionKey) {{ 'selected' }} @endif
                                            @endif>{{ $row->OptionValue }}</option>
                                    @endif
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6 framehideshow">
                        <div class="position-relative form-group">
                            ...
                            <input required type="number" min="0" max="20" id="tollerance"
                    ...
                    <div class="col-md-12">
                        <div class="position-relative form-group d-flex">
                            ...
                            <input type="checkbox" name="Dropseal" id="Dropseal"
                                class="change-event-calulation form-control"
                                style="margin: 2px -4px 10px 12px;border: 1px solid rgb(206, 212, 218);display: inline-block;height: 15px;width: 15px;"
                                value="1" @if (!empty($Item['Dropseal']) && $Item['Dropseal'] == 1) {{ 'checked' }} @endif>
                        </div>
                    </div>
                    <div class="col-md-6 framehideshow">
                        <div class="position-relative form-group">
                    ...
                    <div class="col-md-6 framehideshow">
                        <div class="position-relative form-group">
                            ...
                            <input type="number" min="2" max="4" id="gap" name="gap"
                                ...
                                class="form-control for_c_leaf_height  forleafHeightNoOP change-event-calulation door-configuration forcoreWidth1"
                                required>
                        </div>
                    </div>
                            ...
                            <select id="folder_id" class="form-control" name="FolderId">
                                ...
                                @foreach ($folders as $folderId => $ironmongeries)
                                    <option value="{{ $folderId }}"
                                        @if (isset($Item['FolderId'])) @if ($Item['FolderId'] == $folderId) {{ 'selected' }} @endif
                                        @endif >{{ $ironmongeries->first()->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
...
<input type="hidden" id="currentLeafType"
    value="{{ isset($Item['IntumescentLeafType']) ? $Item['IntumescentLeafType'] : '' }}">
