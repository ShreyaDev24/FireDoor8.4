...
// Frame
    ...
    function framTypeChangeInputEnableDisable(){
        ...
        if (framTypeValue == "Plant_on_Stop") {
            ...
            $("#plantonStopWidth").removeAttr('min', '32');
...
function DoorSetPrice(){
 ...
 //tkt-486
// When a 4 sided frame is used, an additional intumescent needs to be added ‘Pyroplex 8712 brush seal’

$(document).on('change', '#foursidedframe', function(e) {
    if ($(this).is(':checked')) {
        // Show the note when the checkbox is checked
        $('#additional-note').show();
    } else {
        // Hide the note when the checkbox is unchecked
...
// edge value is avilable but got erase when import default
function edgeValue(edge){
    if(edge){
        $("#distanceFromTheEdgeOfDoor").val(edge);
        $('#intumescentSealArrangement').removeAttr('min',)
    }
}
