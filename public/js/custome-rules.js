...
$(document).ready(function() {
    $("#leaf1VisionPanel").change(function(){
        if($(this).val()=="Yes"){
            ...
            $("#distanceFromBottomOfDoor").attr('readonly',false);
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    $(function(){
        ...
        if(overpanelValue == 'No'){
            ...
            $("#opTransomDepth").val('').attr('disabled',true);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function FireRatingChange(){
        if($("#fireRating").val()!=''){
            ...
            filter_fanlight_beads($("#fireRating").val());
            filter_sidelight_beads($("#fireRating").val());
            overpanelOPHeight();
        ...
        $.ajax({
            ...
            success: function(result){
                ...
                if(result.status=="ok"){
                    ...
                    // $("#opGlazingBeads").empty().append(GlazingBeadsInnerHtml);
        ...
        $.ajax({
            ...
            success: function(result){
                if(result.status=="ok"){
                    ...
                    $("#SideLight2GlazingBeadSpeciesIcon").attr("onclick","return  OpenSL2glazingModal('Side Light 2 Glazing Bead Species','SideLight2GlazingBeadSpecies')");
                    $("#SideLight2GlazingBeadSpeciesIcon").addClass("cursor-pointer");
                    $("#SideLight1GlazingBeadSpeciesIcon").attr("onclick","return  OpenSL1glazingModal('Glazing Bead Species','SideLight1GlazingBeadSpecies')");
    ...
    function IntumescentSeals(){
        // alert(55);
        let pageId = pageIdentity();
        const intumescentsealsleaftype = $('#intumescentLeafType').val();       // L,UL
        ...
        if(withoutFrameId == 1){
            sOWidthValue = $('#leafWidth1').val();
            sOHeightValue = $('#leafHeightNoOP').val();
            if(sOWidthValue == '' || sOHeightValue == ''){
                return false;
            }
        }
        // console.log($aa);
    ...
    function DoorLeafFacingChange(status = false,isstatus = false){
        ...
        var DoorLeafFacingValues = document.getElementById('DoorLeafFacingValue-value'); // Get the DOM element

        if (DoorLeafFacingValues != null && isstatus == true) {
            doorLeafFacingValue = $("#DoorLeafFacingValue-value").data("value");
        }
        ...
        $.ajax({
            ...
            success: function(result){
                ...
                var DecorativeGrovesLeaf2Value = document.getElementById('DecorativeGrovesLeaf2-value');
                if(result.status=="ok"){
                    var innerHtml ='';
                    var innerHtml1 ='';
                    var data = result.data;
                    ...
                    for(var index = 0; index<length;index++){
                            ...
                            if(data[index].doorLeafFacing == doorLeafFacing){
                                ...
                                if (DoorLeafFacingValueValue != null && isstatus == true) {
                    ...
                    // }else if(doorLeafFacing=="Veneer" || doorLeafFacing=="Kraft_Paper" || doorLeafFacing=="Playwood"){
                        ...
                        <select required onchange="doorLeafFinishChange();" name="doorLeafFinish" id="doorLeafFinish" option_slug="door_leaf_finish" class="form-control doorLeafFinishSelect">`;
                        innerHtml1+='<option value="">Select Door Leaf Finish</option>';
                        if(color!=''){
                            var length = result.color.length;
                            for(var colorIndex =0; colorIndex<length;colorIndex++){
    ...
    function OpenOPglazingModal(labelname,inputId){
        $('#OPglazingModalLabel').html(labelname);
        $('#inputId').val(inputId);
        $('#OPglazingModal').modal('show');
    }
    function OpenSL1glazingModal(labelname,inputId){
        $('#SL1glazingModalLabel').html(labelname);
        $('#inputId').val(inputId);
        $('#SL1glazingModal').modal('show');
    }
    function OpenSL2glazingModal(labelname,inputId){
        $('#SL2glazingModalLabel').html(labelname);
        $('#inputId').val(inputId);
        $('#SL2glazingModal').modal('show');
    }
...
// Not relate to any one
    $("#overpanel1").change(function(){
        ...
        if($("#overpanel").val()=="No"){
            ...
            $("#opTransomDepth").val('').attr('disabled','disabled');
            ...
            $("#opTransomDepth").val('').attr('disabled',false);
    ...
    $(".for_Leaf_Width_1").change(function(){
        ...
        if($("#doorsetType").val()=="DD" ){
            ...
            var GrooveLocationLeaf2Value = $('#GrooveLocationLeaf2').val();
            if(GrooveLocationLeaf2Value == "Vertical" && leafWidth1 != ''){
                $("#MaxNumberOfGrooveLeaf2").val(Math.round((parseInt(leafWidth1)/100)));
            }
...
function glazing_system(isIntegrity,isstatus = false){
    ...
    $.ajax({
        ...
        success: function(result){
            ...
            var selectElement = $("#glazingBeads").empty(); // Assuming the select element has this ID
            var selectElement1 = $("#opGlazingBeads").empty(); // Assuming the select element has this ID
            var selectElement2 = $("#SideLight1BeadingType").empty(); // Assuming the select element has this ID
            var selectElement3 = $("#SideLight2BeadingType").empty(); // Assuming the select element has this ID
            ...
            decodedBeads.forEach(function(bead) {
                var cleanedBead = bead.trim();
                var valueWithUnderscores = cleanedBead.replace(/ /g, '_');
                var option = $('<option></option>')
                    .val(valueWithUnderscores) // Set value with underscores
                    .text(bead); // Set display text without underscores
            ...
            if (savedValue && isstatus == true) {
                selectElement.val(savedValue);
                selectElement1.val(savedValue);
...
// $(".intumescentLeafchange").change(function() {
//     var leafTypeId = $(this).val();  // Get the selected leaf type ID
//     intumescentLeafChange(leafTypeId);  // Pass the value to the function
// });



$(document).ready(function() {
    // Detect if it's the edit page by checking if current values exist (this can be done with Blade in Laravel)
    var currentLeafType = $('#currentLeafType').val();
    var currentFireRating = $('#currentFireRating').val();;

    // If it's an edit page and there's a preselected leaf type
    if (currentLeafType) {
        // Set the leaf type in the dropdown
        $('#intumescentLeafType').val(currentLeafType);

        // Trigger the function to load fire ratings based on the selected leaf type
        intumescentLeafChange(currentLeafType);

        // After the fire ratings are loaded, set the selected fire rating
        // $(document).ajaxComplete(function() {
            $('#fireRating').val(currentFireRating);  // Preselect the fire rating after options are populated
        // });
    }

    // On both add and edit pages, handle the change event when a user selects a new leaf type
    $(".intumescentLeafchange").change(function() {
        var leafTypeId = $(this).val();  // Get the selected leaf type ID
        intumescentLeafChange(leafTypeId);  // Load fire ratings for the selected leaf type
    });
});

function intumescentLeafChange(leafTypeId) {
    if (leafTypeId !== '') {
        // Make AJAX request to get fire rating options based on the leaf type
        $.ajax({
            url: '/quotation/get-fire-rating-options',  // Your endpoint to fetch fire rating options
            method: 'GET',
            data: { leaf_type_id: leafTypeId },  // Send the selected leaf type ID to the server
            success: function(response) {
                // Clear the fire rating select box before adding new options
                $('#fireRating').empty();
                $('#fireRating').append('<option value="">Select Fire Rating</option>');

                // Iterate through the fire ratings and append them to the select box
                $.each(response.fire_ratings, function(index, fireRating) {
                    $('#fireRating').append('<option value="' + fireRating.OptionKey + '">' + fireRating.OptionKey + '</option>');
                });

                var importfireRating = $("#FireRating-import").data("value");
                if(importfireRating != null){
                    $("select[name=fireRating]").val(importfireRating);
                }

                // If there is a currently selected fire rating, set it
                if ($('#currentFireRating').val()) {
                    $('#fireRating').val($('#currentFireRating').val());
                }
                ...
                doorleaffacing(response.leafType);
        ...
        // Reset fire rating select if no leaf type is selected
        $('#fireRating').empty();
        $('#fireRating').append('<option value="">Select Fire Rating</option>');
    }
}

function doorThicknessLeaf(values) {
    // $("#door_thickness_div").empty().append(`<input type="number" readonly name="doorThickness" id="doorThickness" class="form-control" value="${values.door_thickness}">`);
    const doorThicknessOptions = `<input type="number" readonly name="doorThickness" id="doorThickness" class="form-control" value="${values.door_thickness}">`;

    // Clear and update the door thickness div for all cases
    $("#door_thickness_div").empty().append(doorThicknessOptions);
 }

// function doorleaffacing(values){
//   var doorLeafFacingSelect = document.getElementById('doorLeafFacing');
//         // Clear all options first
//     var options = doorLeafFacingSelect.options;
//     for (var i = options.length - 1; i >= 1; i--) { // Start at index 1 to keep the first "Select door leaf facing" option
//         doorLeafFacingSelect.remove(i);
//     }
//         // Add options based on the selected leaf type
//     if (values.id === 1 || values.id  === 2 || values.id  === 5 || values.id === 6) {
//         addOption(doorLeafFacingSelect, "Kraft Paper", "Kraft_Paper");
//         addOption(doorLeafFacingSelect, "Veneer", "Veneer");
//         addOption(doorLeafFacingSelect, "Laminate", "Laminate");
//     } else if (values.id === 3 || values.id === 4) {
//         addOption(doorLeafFacingSelect, "PVC", "PVC");
//     }
// }
...
function doorleaffacing(values) {
    ...
    var selectedValue = doorLeafFacingSelect.value;
    ...
    if (values.id === 1 || values.id === 2 || values.id === 5 || values.id === 6) {
        ...
        addOption(doorLeafFacingSelect, "Veneer", "Veneer");
        addOption(doorLeafFacingSelect, "Laminate", "Laminate");
        addOption(doorLeafFacingSelect, "Raw", "Raw");
    } else if (values.id === 3 || values.id === 4) {
        addOption(doorLeafFacingSelect, "PVC", "PVC");
    }

    // Reapply the previously selected value if it still exists
    for (var i = 0; i < doorLeafFacingSelect.options.length; i++) {
        if (doorLeafFacingSelect.options[i].value === selectedValue) {
            doorLeafFacingSelect.options[i].selected = true;
            break;
        }
    }
}
...
// JFDS-659
function filter_fanlight_beads(fireRating){
    let pageId = pageIdentity();
        $.ajax({
            url: $("#fanlightBeadsFilter").html(),
            method:"GET",
            dataType:"Json",
            data:{pageId:pageId,fireRating:fireRating,_token:$("#_token").val()},
            success: function(result){
                var OPGlazingBeadsInnerHtml = '';
                if(result.status=="ok"){
                    var data = result.data;
                    var length = result.data.length;
                    var OPGlazingBeadsValue = document.getElementById('OPGlazingBeads-value');
...
// JFDS-658
function filter_sidelight_beads(fireRating){
    let pageId = pageIdentity();
        $.ajax({
            url: $("#sidelightBeadsFilter").html(),
            method:"GET",
            dataType:"Json",
            data:{pageId:pageId,fireRating:fireRating,_token:$("#_token").val()},
            success: function(result){
                var SideLight1BeadingTypeInnerHtml = '', SideLight2BeadingTypeInnerHtml = '';
                if(result.status=="ok"){
                    var data = result.data;
                    var length = result.data.length;
                    var BeadingTypeValue = document.getElementById('BeadingType-value');
...
// JFDS-700
$("#SL1Transom").change(function(){
    if($(this).val()=="No"){
        $("#SL1TransomDepth").removeAttr('required',false);
        $("#SL1TransomDepth").attr('disabled',true);
    }
});
$("#SL2Transom").change(function(){
    if($(this).val()=="No"){
        $("#SL2TransomDepth").removeAttr('required',false);
        $("#SL2TransomDepth").attr('disabled',true);
    }
});
...
function LippingIns(fireratings){
    if(fireratings == 'NFR'){
        $('#lippingType').removeAttr('required')
...
// Vision Panel New Validations
function VisionPanelValidations(firerating){
    let visionPanelQuantityRule = $("#visionPanelQuantity").val();
    let visionPanelQuantityforLeaf2Rule = $("#visionPanelQuantityforLeaf2").val();
    if(firerating == "FD30" || firerating == "FD30s"){
        $("#distanceFromTopOfDoor").attr("min",100);
        $("#distanceFromBottomOfDoor").attr("min",100);
        $("#distanceFromTheEdgeOfDoor").attr("min",100);
        $("#distanceBetweenVPs").attr("min",80);
        if(visionPanelQuantityRule == 1){
            $("#distanceBetweenVPs").removeAttr("min",80);
        }
        if(visionPanelQuantityforLeaf2Rule == 1){
