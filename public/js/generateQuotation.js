$(function(){
    ...
    ElevationDrawing = function() {
        ...
        if (currentVersion != 0) {
            // if(bomTag > 0){
            let url = printInvoiceUrl + '/' + quotationId + '/' + currentVersion + '/' + isActive;
            var request = new XMLHttpRequest();
            request.onreadystatechange = function() {
                if(request.readyState == 4) {
                    if(request.status == 200) {
                        ...
                        var link = document.createElement('a');
                        link.href = window.URL.createObjectURL(blob);
                        link.download = "ElevationDrawing "+generatedId.replace('#','')+"-"+versionnumber+".pdf";
                        link.click();
                        $('.loader').css({'display':'none'});
                    } else if(request.responseText != "") {
                        $('.loader').css({'display':'none'});
                    ...
                    if(request.status == 200) {
                        ...
                        var link = document.createElement('a');
                        link.href = window.URL.createObjectURL(blob);
                        link.download = "ElevationDrawing"+generatedId+"-"+versionnumber+".pdf";
                        link.click();
                        $('.loader').css({'display':'none'});
                    } else {
                        request.responseType = "text";
