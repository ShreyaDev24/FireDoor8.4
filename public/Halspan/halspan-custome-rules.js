...
$(document).ready(function() {
    ...
    $(document).ready(function () {
        ...
        if(selectedValue == 'Rebated_Frame'){
            ...
            let width = $("#rebatedwidthnew").val();
            let height = $("#rebatedHeightnew").val();
            $("#rebatedWidth").val(width);
            $("#rebatedHeight").val(height);
        }
        if ($('#frameType').val() !== selectedValue) {
            $('#frameType').val('');
        }
    });
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function IntumescentSeals(){
        ...
        if(fireRating != '' && sOWidthValue != '' && sOHeightValue != '' || intumescentSealType != ''){
            $.ajax({
                ...
                method:"POST",
                dataType:"Json",
                data:{pageId:pageId,SelectedValue:SelectedValue,fireRating:fireRatingValue,intumescentseals: $aa ,leafWidth1Value:leafWidth1Value, leafHeightNoOPValue:leafHeightNoOPValue,doorLeafFacingValueNew:doorLeafFacingValueNew,frameMaterialNew:frameMaterialNew,intumescentsealsleaftype:intumescentsealsleaftype,intumescentSealType:intumescentSealType ,_token:$("#_token").val()},
                success: function(result){
                    // console.log(result);
    ...
    /*
     | Vision Panel certificate limits (Halspan Optima FD30).
     | Each glass + glazing combination's max area/width/height are attached as
     | data-* attributes on the #glazingSystems <option> (data-vparea m²,
     | data-vpwidth mm, data-vpheight mm) and shown in the hover info box.
     | Size enforcement is done server-side by filtering the glass/glazing lists
     | (see ItemListController glassGlazingFilter / GlazingFilter).
     */
        ...
        success: function(result){
            ...
            if(result.status=="ok"){
                ...
                for(var i =0; i<length;i++){
                    ...
                    // console.log(data[i].GlassThickness);
            ...
            success: function(result){
                ...
                if(result.status=="ok"){
                    ...
                    if(type == "opGlassType"){
                        ...
                        $("#opglazingSystemsThickness").val(data.GlazingThickness);
                      //  $("#opGlazingBeads").val(data.Beading);
...
function VisionPanelValidations(firerating){
    ...
    if(firerating =="FD30" || firerating =='FD30s'){
        $("#distanceFromTopOfDoor").attr("min",90);
