...
window.renderHalspanFamily = (CustomElement = null,formValues = {}) => {
    ...
    // Check if CustomElement is a jQuery object and extract the DOM element
    if (CustomElement instanceof jQuery) {
        element = d3.select(CustomElement.get(0)); // or CustomElement[0]
        ChangedFieldName = element.attr("name");
    } else if (CustomElement instanceof Element) {
        element = d3.select(CustomElement);
        ChangedFieldName = element.attr("name");
    }
    ...
    var DoorSetType = formValues.DoorsetType;
    if (DoorSetType == "") {
        DoorSetType = 0;
    }

    var swingType = formValues.SwingType;
    if (swingType == "") {
        swingType = 0;
    }

    var SOWidthForMap = 0;
    var SOWidth = formValues.SOWidth;;
    var withoutFrameId = formValues.FrameOnOff;
    if(formValues.FourSidedFrame == 1){
        var IsFourSidedFrame = true;
    }

    if (withoutFrameId == 1) {
       var SOWidth = parseInt(formValues.LeafWidth1, 10);
        if (DoorSetType == "DD") {
            SOWidth = SOWidth * 2;
        } else if (DoorSetType == "leaf_and_a_half") {
            SOWidth = parseInt(formValues.LeafWidth1, 10) + parseInt(formValues.LeafWidth2, 10);
        }
    }
    if (SOWidth == "") {
        SOWidth = 0;
    ...
    var SOHeight = formValues.SOHeight;
    if (withoutFrameId == 1) {
        var SOHeight = formValues.LeafHeight;
    }
    if (SOHeight == "") {
        SOHeight = 0;
    } else {
 ...
 // console.log(SOHeight)
    ...
    if (SOWidth > 0 && SOHeight > 0) {
        ...
        var Tollerance = formValues.Tollerance;
        if (Tollerance == "") {
            Tollerance = 0;
        } else {
        ...
        var FrameThickness = formValues.FrameThickness;
        ...
        var FloorFinish = formValues.FloorFinish;
        if (FloorFinish == "") {
            FloorFinish = 0;
        } else {
            FloorFinish = parseFloat(FloorFinish);
        }
        swingType = formValues.SwingType;
        if (swingType == "") {
            swingType = 0;
        }
        ...
        var hingeLocation1 = (formValues.hinge1Location > 0) ? parseInt(formValues.hinge1Location, 10) : 0;
        var hingeLocation2 = (formValues.hinge2Location > 0) ? parseInt(formValues.hinge2Location, 10) : 0;
        var hingeLocation3 = (formValues.hinge3Location > 0) ? parseInt(formValues.hinge3Location, 10) : 0;
        var hingeLocation4 = (formValues.hinge4Location > 0) ? parseInt(formValues.hinge4Location, 10) : 0;
        var hingeCenter = formValues.hingeCenterCheck ? 1 : 0;

        // // console.log(hingeLocation1,hingeLocation2,hingeLocation3,hingeLocation4)
        ...
        ConfigurableDoorFormula.forEach(function (elem, index) {
            ...
            // console.log(UnderCutAdditionalNumber)
        ...
        var FrameWidth = formValues.FrameWidth;
        ...
        var FrameHeight = formValues.FrameHeight;
        ...
        var UnderCut = 0;
        // console.log(FloorFinish,'jiiii')
        if (FloorFinish > 0) {
            if (formValues.FireRating == 'FD30' || formValues.FireRating == 'FD60') {
                UnderCut = FloorFinish + 8;
            } else if (formValues.FireRating == 'FD30s' || formValues.FireRating == 'FD60s') {
                UnderCut = FloorFinish + 3;
            } else {
                UnderCut = formValues.Undercut
            }
        }
        ...
        if (DoorSetType == "DD" && withoutFrameId != 1) {
            LeafWidth1 = LeafWidth2 = formValues.LeafWidth1;
        } else if (DoorSetType == "SD" && withoutFrameId != 1) {
            LeafWidth1 = formValues.LeafWidth1;;
        } else if (DoorSetType == "leaf_and_a_half") {
            if (withoutFrameId != 1) {
                LeafWidth1 = parseInt(formValues.LeafWidth1, 10);
                if (LeafWidth1 != "") {
                    LeafWidth1 = parseFloat(LeafWidth1);
                }
                LeafWidth2 = formValues.LeafWidth2;
            }
            DoorSetType = "DD";
        }

        if (withoutFrameId == 1) {
            var LeafHeightNoOP = formValues.LeafHeight;
            var SOHeight = formValues.LeafHeight;
            var SOWidth = parseInt(formValues.LeafWidth1, 10);
            var LeafWidth1 = parseInt(formValues.LeafWidth1, 10);
            if (DoorSetType == "DD") {
                var LeafWidth2 = parseInt(formValues.LeafWidth1, 10);
            } else {
                var LeafWidth2 = parseInt(formValues.LeafWidth2, 10);
            }

            if (formValues.DoorSetType == 'leaf_and_a_half') {
                var LeafWidth2 = parseInt(formValues.LeafWidth2, 10);
        ...
        if (formValues.DoorSetType == 'leaf_and_a_half') {
            ...
            // console.log(LeafWidth1ForMap,'test',LeafWidth1)
        ...
        if (formValues.FourSidedFrame == 1) {
            LeafHeightNoOP = formValues.LeafHeight;
        }
        if (LeafHeightNoOP == "") {
            LeafHeightNoOP = 0;
        } else {
            //LeafHeightNoOPForMap = NumberChanger(LeafHeightNoOP);
            LeafHeightNoOPForMap = (Number(LeafHeightNoOP) + UnderCut) / 5;
        }
        // console.log(LeafHeightNoOPForMap,'thtsts')
        ...
        // // console.log(Tollerance , TolleranceAdditionalNumber,FrameThickness , FrameThicknessAdditionalNumber,GapAdditionalNumber ,Gap)
        ...
        if (DoorSetType == "DD") {
           ...
           // console.log(FrameThicknessForMap,GapForMap,LeafWidth1ForMap,GapForMap,FrameThicknessForMap,TotalDoorWidth,'me')
        // // console.log(TotalDoorWidth)
        ...
        var SideLightPanel1 = formValues.SideLight1
        if (SideLightPanel1 == "Yes") {
            SideLightPanel1Width = formValues.SL1Width;
            ...
            var SideLightPanel1Height = formValues.SL1Height;
        ...
        var SideLightPanel2 = formValues.SideLight2;
        if (SideLightPanel2 == "Yes") {

            var CopyOfSideLite1 = formValues.DoYouWantToCopySameAsSL1;

            if (CopyOfSideLite1 != "Yes") {

                SideLightPanel2Width = formValues.SL2Width;
                ...
                SideLightPanel2Height = formValues.SL2Height;
        ...
        var IsOverPanelActive = formValues.Overpanel;
        if (IsOverPanelActive != "" && IsOverPanelActive != "No") {
            ...
            var OverPanelHeight = formValues.OPHeigth;
            ...
            //     OverPanelHeight = 0;
            // }
            // console.log(formValues.oPHeigth)
        ...
        // console.log(hngLctn1Y,hngLctn2Y)
...
`)
        ...
        svg.append('rect')
            ...
            .attr('stroke', 'black')
            .attr('fill', '#EBECE6');
   // console.log(FrameThicknessForMap,GapForMap,TopFrameHeight,LeafWidth1ForMap,LeafHeightNoOPForMap,'hiii')
        ...
        if ((swingType && swingType == 'DA') || !frameonoff) {
            ...
            svg.append('rect')
                ...
                .attr('stroke', 'black')
                .attr('fill', '#C8C8BC');
                console.log(ix + FrameThicknessForMap - 1 ,hngLctn1Y)
            // // console.log("925", GapForMap, hngLctn1Y, ix + FrameThicknessForMap)
            svg.append('rect')
                .attr('x', ix + FrameThicknessForMap - 1)
                // .attr('y', iy + TopFrameHeight + GapForMap + 100)
        ...
        if (IronmongerySet == "Yes" && IronmongeryID != "") {
            ...
            ParsedIronmongerySet.forEach(function (elem, index) {
                if (elem.id == IronmongeryID) {
                    ...
                    // additional_info isn't present on the raw ironmongery data this page
                    // loads (unlike the per-item add/edit page), so every lookup below is
                    // guarded with ?. instead of assuming it exists - a missing category
                    // just means that accessory isn't drawn, rather than crashing render()
                    // and silently killing the whole item's Validate All save.
        ...
        if (DecorativeGroves == "Yes") {
            ...
            var GrooveLocation = formValues.GrooveLocation;
            ...
            if (GrooveLocation == "Vertical" || GrooveLocation == "Horizontal") {
                var NumberOfGroove = formValues.NumberOfGroove;
                if (NumberOfGroove != "") {
            ...
            if (GrooveLocation == "Vertical_&_Horizontal") {
                ...
                var NumberOfVerticalGroove = formValues.NumberOfVerticalGroove;
                if (NumberOfVerticalGroove != "") {
                    NumberOfGrooveForVertical = parseFloat(NumberOfVerticalGroove) + 1;
                }

                var NumberOfHorizontalGroove = formValues.NumberOfHorizontalGroove;
        ...
        var Leaf1VisionPanel = formValues.Leaf1VisionPanel;

        var Leaf1VisionPanelWidth = formValues.Leaf1VPWidth;
        ...
        var distanceBetweenVP = formValues.DistanceBetweenVp;
        var DistanceBetweenVPsMinValue = 80;
        var DistanceFromTopOfDoorValue = formValues.DistanceFromtopOfDoor;
        var DistanceFromTopOfDoorMinValue = 100;
        var DistanceFromTheEdgeOfDoorMinValue = 100;
        ...
        if (Leaf1VisionPanel == "Yes") {
            if (ChangedFieldName == "leaf1VisionPanel") {
                ...
                DistanceFromTopOfDoorForLeaf1 = formValues.DistanceFromtopOfDoor;
                ...
                DistanceFromTheEdgeOfDoorForLeaf1 = formValues.DistanceFromTheEdgeOfDoor;
        ...
        var VisionPanelShape = formValues.Leaf1VisionPanelShape;

        var AreVPsEqualSizesForLeaf1 = formValues.AreVPsEqualSizesForLeaf1;

        var VisionPanelQuantityForLeaf1 = formValues.VisionPanelQuantity;
        ...
        if (Leaf1VisionPanel == "Yes") {
            if (ChangedFieldName == "visionPanelQuantity" && VisionPanelQuantityForLeaf1 > 1) {
                ...
                if (VisionPanelQuantityForLeaf1 < 2) {
                    ...
                    DistanceBetweenVPsForLeaf1 = formValues.Leaf1VisionPanelShape;
        ...
        if (Leaf1VisionPanel == "Yes") {
            if (ChangedFieldName == "distanceBetweenVPs") {
                DistanceBetweenVPsForLeaf1 = formValues.DistanceBetweenVPs;
                if (DistanceBetweenVPsForLeaf1 == "" || parseFloat(DistanceBetweenVPsForLeaf1) < DistanceBetweenVPsMinValue) {
                    DistanceBetweenVPsForLeaf1 = DistanceBetweenVPsMinValue;
                }
            }
...
conditionalRenderItem(thumbturnDistance, thumbturnDistance * 5, 0, 255, 250,285,290);
        ...
        if (Leaf1VisionPanel == "Yes") {
            ...
            if (AreVPsEqualSizesForLeaf1 != "Yes") {
                ...
                Leaf1VisionPanel1Height = formValues.Leaf1VPHeight1;
                ...
                Leaf1VisionPanel2Height = formValues.Leaf1VPHeight2;
                ...
                Leaf1VisionPanel3Height = formValues.Leaf1VPHeight3;
                ...
                Leaf1VisionPanel4Height = formValues.Leaf1VPHeight4;
                ...
                Leaf1VisionPanel5Height = formValues.Leaf1VPHeight5;
                ...
                var VP1Height1 = formValues.Leaf1VPHeight1;
                if (VP1Height1 == "") {
                    VP1Height1 = 0;
                } else {
        ...
        if (DoorSetType == "DD") {
            ...
            var Leaf2VisionPanel = formValues.Leaf2VisionPanel
            if (Leaf2VisionPanel == "Yes") {
                ...
                if (VPSameAsLeaf1 != "Yes") {
                    ...
                    Leaf2VisionPanelWidth = formValues.Leaf2VPWidth;
                    ...
                    AreVPsEqualSizesForLeaf2 = formValues.AreVPsEqualSizesForLeaf2;
                    VisionPanelQuantityForLeaf2 = formValues.Leaf2VisionPanelQuantity;
                    ...
                    if (AreVPsEqualSizesForLeaf2 == "No") {
                        ...
                        Leaf2VisionPanel1Height = formValues.Leaf2VPHeight1;
                        ...
                        Leaf2VisionPanel2Height = formValues.Leaf2VPHeight2;
                        ...
                        Leaf2VisionPanel3Height = formValues.Leaf2VPHeight3;
                        ...
                        Leaf2VisionPanel4Height = formValues.Leaf2VPHeight4;
                        ...
                        Leaf2VisionPanel5Height = formValues.Leaf2VPHeight5;
                        ...
                        var VP2Height1 = formValues.Leaf2VPHeight1;
                        if (VP2Height1 == "") {
                            VP2Height1 = 0;
                        } else {
        ...
        if (ShowMeasurements) {
            ...
            if(IsOverPanelActive != "" && IsOverPanelActive != "No"){
                var THeight = Number(FrameHeight) + Number(OverPanelHeightToShow) + Number(FrameThickness);
                console.log(THeight)
                if(THeight >= 2950){
                ...
                svg.append('line') //height with overpanel of door
                    ...
                    .attr("marker-start", "url(#arrowLeft)")  // Left-pointing arrow
                    .attr("marker-end", "url(#arrowRight)")
                    console.log(FrameHeightForMap , OverPanelHeight)
                ...
                svg.append("text") // append text
                    ...
                    .text(Number(FrameHeight) + Number(OverPanelHeightToShow)); // define the text to display
                    console.log(FrameHeight,OverPanelHeightToShow)
        ...
        setTimeout(() => {
            ...
            // 🔁 Call run with the correct item (formValues)
            run(encodedData, formValues);
...
function run(svgData,item) {
    ...
    imageUtil.base64SvgToBase64Png(svgSrc, 200).then(pngSrc => {
        ...
        saveCadImage(pngSrc,item);
        //document.getElementById('pngImage').src = pngSrc;
    });
}

function saveCadImage(pngSrc, item) {
    var itemID = item.itemId;
    var SvgImage = pngSrc;
    var versionId = $('#versionId').val();
    var leafWidth1 = item.LeafWidth1;
            ...
            success: function(res) {
                ...
                if (res.status == 'error') {
                    ...
                    if (completedItems === totalItemsToSave) {
                        ...
                        swal({
                            title: "Validate!",
                            text: "All doors validated successfully",
                            type: "success"
                        }).then(function () {
                            location.reload();
                        });
