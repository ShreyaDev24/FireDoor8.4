...
$("#DoorDimensionsIcon2").on("click", function () {
    ...
    $.ajax({
        url: url,
        type: "GET",
        data: { door_leaf_facing: door_leaf_facing, door_leaf_finish: door_leaf_finish, leaf_type: leaf_type, firerating, firerating, page_id: 4 ,DoorDimensionId:DoorDimensionId},
        success: function (data) {
            var result = data.map((row) => {
            let displayCode = row.code
                ? `${row.code}`
