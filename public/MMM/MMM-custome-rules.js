...
function DoorDimensionGroovesIcon() {
   ...
   let pageId = 9;
    if (decorativeGroves == 'Yes' && pageId == 9) {
...
function filterSpecies(){
    ...
    $.ajax({
        url: url,
        type: "GET",
        data: { door_leaf_facing: door_leaf_facing, door_leaf_finish: door_leaf_finish, leaf_type: leaf_type, firerating, firerating, page_id: 9 },
            ...
            if (result.status == "ok") {
                ...
                if (doorLeafFacing == "Laminate" || doorLeafFacing == "PVC") {
                    if (doorLeafFacing == "Laminate") {
                        ...
                        $("#decorativeGroves,#DecorativeGrovesLeaf2").attr({ 'disabled': false });
                    }
...
function architrave(isModal=0) {
    ...
    $.ajax({
        ...
        method: "POST",
        dataType: "Json",
        data: {  _token: $("#_token").val(), pageId: 9 },
        success: function (result) {
            if (result.status == "ok") {
...
$("#DoorDimensionsIcon").on("click", function () {
    ...
    console.log(doorThickness,firerating)
    $.ajax({
        url: url,
        type: "GET",
        data: { door_leaf_facing: door_leaf_facing, door_leaf_finish: door_leaf_finish, leaf_type: leaf_type, firerating, firerating, page_id: 9 },
...
$("#DoorDimensionsIcon2").on("click", function () {
    ...
    $.ajax({
        url: url,
        type: "GET",
        data: { door_leaf_facing: door_leaf_facing, door_leaf_finish: door_leaf_finish, leaf_type: leaf_type, firerating, firerating, page_id: 9 ,DoorDimensionId:DoorDimensionId},
        success: function (data) {
            var result = data.map((row) => {
            let displayCode = row.code
                ? `${row.code}`
            ...
            return `
                <div class="col-md-2 col-sm-4 col-6 cursor-pointer" data-dismiss="modal" onclick="DoorDimensionValueFill2(${row.id},'${displayCode}',${row.mm_width},${row.mm_height})"><div class="color_box"><div class="frameMaterialImage"><img width="100%" height="100" src="${base_url}/DoorDimension/${row.image ? row.image : "vicaima_default_doorDimantion.jpg"}"></div><h4>${displayCode}-${row.mm_width}x${row.mm_height}</h4></div></div>`
            });
...
function doorDimensionCalculation(){
   ...
   //  Door leaf W -leaf width  adjustment + gapx2+ToleranceX2+Frame thicknessX2
    ...
    // var soHeight = parseInt(leafHeightNoOP) + parseInt(tollerance) + parseInt(frame_thickness) + parseInt(undercut) + parseInt(gap); // old formula
...
$(document).ready(function(){
    $("#overpanel").change(function () {
        if($(this).val() == 'Overpanel'){
            $("#opGlassIntegrity").attr({ 'disabled': true, required: false }).val('');
            $("#opGlassType").attr({ 'disabled': true, required: false }).val('');
            $("#opglazingSystems").attr({ 'disabled': true, required: false }).val('');
            $("#opGlazingBeads").attr({ 'disabled': true, required: false }).val('');
            $("#opglazingBeadsThickness").attr({ 'disabled': true, required: false }).val('');
            $("#opglazingBeadsHeight").attr({ 'disabled': true, required: false }).val('');
            $("#opglazingBeadsFixingDetail").attr({ 'disabled': true, required: false }).val('');
            // JFDS 896
    ...
    if(overpanelVal == 'Overpanel'){
        $("#opGlassIntegrity").attr({ 'readonly': true, required: false }).val('');
        $("#opGlassType").attr({ 'disabled': true, required: false }).val('');
        $("#opglazingSystems").attr({ 'disabled': true, required: false }).val('');
        $("#opGlazingBeads").attr({ 'disabled': true, required: false }).val('');
        $("#opglazingBeadsThickness").attr({ 'disabled': true, required: false }).val('');
...
// 905
function myfunction(){
    if ($('#checking-level').css('display') !== 'none') {
...
$('#oPHeigth,#SL1Width').on('input', function () {
    // let overpanel = $('#overpanel').val(); // get dynamic value from the UI reverted code
    // var sideLight1 = $("#sideLight1").val();
    // var sideLight2 = $("#sideLight2").val();
    // if(overpanel != 'No'){
    //     UpdateformulaSoHeightAndWidth(overpanel)  reverted code
    // }
    // if(sideLight1 == 'Yes'){
    //     updateformulaS1(sideLight1)
    // }
...
// function UpdateformulaSoHeightAndWidth(val){ reverted code
...
//         var so_width = parseInt(leafWidth1)  - parseInt(adjustmentLeafWidth1)  + parseInt(gap) + parseInt(undercut) + parseInt(tollerance) + parseInt(frame_thickness) + parseInt(oPHeigth);
//         var soHeight = parseInt(leafHeightNoOP) - parseInt(adjustmentLeafHeightNoOP) + parseInt(gap) + parseInt(undercut) + parseInt(frame_thickness) +  parseInt(tollerance) + parseInt(oPHeigth);
...
function overpanelcheck(){
    ...
    if(overpanels == 'No'){
        $("#opglazingSystems").attr('disabled',true)
