...
class SuperAdminController extends Controller
{
    ...
    public function store(request $request)
    {
        ...
        if(!empty($request->FirstName) && !empty($request->LastName) &&  !empty($request->UserEmail) &&  !empty($request->UserPhone) && !empty($request->UserJobtitle)){
            ...
            if ($request->hasFile('UserImage')) {
                ...
                $filepath = public_path('SuperAdminLogo');
    ...
    public function list()
    {
        ...
        if (Auth::user()->UserType == '1') {
            $data = User::where('UserType', 2)->orWhere('is_superadmin',1)->where('CreatedBy', Auth::user()->id)->orderBy('id', 'desc')->get();
            dd($data);

            return view('SuperAdmins.list', ['data' => $data]);
        }
    ...
    public function details($id)
    {
        if (Auth::user()->UserType == '1') {
            if (isset($id)) {
                ...
                return view('SuperAdmins.details', ['data' => $data]);
            } else {
                return redirect()->route('user/list');
            }
        } else {
