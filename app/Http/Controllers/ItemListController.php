...
class ItemListController extends Controller
{
            ...
            $updateDetails = [
                //item (update)
                    ...
                    //Over Panel Section
                        ...
                        'FanLightWidthGlass'                    => $request->fanlightWidthGlass,
                        'FanLightHeightGlass'                   => $request->fanlightHeightGlass,
                        'OpPanelWidth'                          => $request->opPanelWidth,
                        'OpPanelHeight'                         => $request->opPanelHeight,
                    ...
                    //Side Light
                        ...
                        'SideLight1FrameThickness'              => $request->sideLight1FrameThickness,
            ...
            if($mm > 0){
                ...
                // item (insert)
                    ...
                    //Over Panel Section
                        ...
                        $item->OpPanelWidth = $request->opPanelWidth;
                        $item->OpPanelHeight = $request->opPanelHeight;
                        $item->FanLightWidthGlass = $request->fanlightWidthGlass;
                        $item->FanLightHeightGlass = $request->fanlightHeightGlass;
        ...
        if(!empty($leafWidth1Value) && !empty($leafHeightNoOPValue)){
            ...
            if(in_array($UserType,[1,4])){
                ...
                */
                ...
                if (!empty($request->leafWidth1Value) && !empty($request->leafHeightNoOPValue) && !empty($request->intumescentSealType)) {
                    $getConditions = array_merge($getConditions, [
                        ['setting_intumescentseals2.FireOnly', $request->intumescentSealType]
                    ]);
                }
    ...
    public function glassGlazingFilter(Request $request): void{
        ...
        if ($configurationDoor == 'Halspan' || $configurationDoor == 'Streboard') {
    ...
    public function GlazingFilter(Request $request): void{
        ...
        if(!empty($glassType)){
            ...
            if ($configurationDoor == 'Halspan' || $configurationDoor == 'Streboard' || $fireRating !== 'NFR'|| ($configurationDoor == 'VicaimaDoorCore' && $fireRating == 'FD60')) {
            ...
            if(!empty($glassType)){
                ...
                if ($configurationDoor == 'Halspan' || $configurationDoor == 'Streboard' || ($configurationDoor == 'VicaimaDoorCore' && $fireRating == 'FD60')) {
                    if (!empty($leaf1VpWidth))  $query->where('glass_glazing_system.VPWidth',  '>=', $leaf1VpWidth);
                    if (!empty($leaf1VpHeight)) $query->where('glass_glazing_system.VPHeight', '>=', $leaf1VpHeight);
                }
